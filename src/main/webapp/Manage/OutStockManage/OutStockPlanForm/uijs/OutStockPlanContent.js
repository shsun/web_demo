...
var gridwindow = Ext
		.create(
				'Ext.grid.Panel',
				{
					store : OutStockPlanContentStore,
					layout : 'fit',
					id : 'listwindow',
											...
											field : {
												...
												listeners : {
													select : function(combox,
														...
														// alert(combox.value)
														// LocationStore.proxy=new
														// Ext.data.HttpProxy({
														// url:
														// '/WebWms/outStockPlanContent/cbb_list.action?fk_goods_pk='+combox.value,
														// //保存
														// });//动态显示老师姓名
														// LocationStore.load();
					...
					bbar : new Ext.PagingToolbar({
						xtype : 'paging',
						height : '30',
						store : 'OutStockPlanContentStore', // 主要是取store中从后台返回的totalCount，以及在store中设置的pagesize，就可以进行分页。具体的数据分页在后台的sql语句中进行。
...
var win_add = function(btn) {
	...
	var r = Ext.create(OutStocktPlanContentModel, {});
...
var win_save = function(btn) {
	...
	if (records.length > 0) {
		...
		if (jsonArray.length > 0) {
			Ext.Ajax.request({
				url : '/WebWms/outStockPlanContent/save.action', // 保存
				method : 'POST',
				params : {
	...
	if (len > 0) {
		...
		Ext.Msg.show({
			...
			fn : function(btn) {// 用户点击交互后调用的函数，btn是指用户点了那个按钮
				if (btn === 'yes') {
					Ext.Ajax.request({
						url : '/WebWms/outStockPlanContent/delete.action', // 保存
						params : {
							'ids' : ids
						},
						success : function(response, options) {
