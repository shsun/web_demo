		...
		.create(
				'Ext.grid.Panel',
				{
					...
					columns : [
							...
							{
								text : '出库计划单号',
								dataIndex : 'outstockplanform_number',
								align : 'center',
								editor : {
									allowBlank : false,
									emptyText : '请填写出库计划单号'
							...
							{
								text : '创单日期',
								dataIndex : 'outstockplanform_date',
								align : 'center',
								xtype : 'datecolumn',
								format : 'Y-m-d',
								field : {
							...
							{
								text : '备注',
								dataIndex : 'outstockplanform_remark',
								align : 'center',
								editor : {
									allowBlank : true
								}
							...
							{
								text : '状态',
								dataIndex : 'outstockplanform_state',
								align : 'center',
								// editor:{
								// allowBlank:true
								// },
								renderer : function(value) {
									return "<span style='margin-right:10px'><a href='#' onclick='win()'  >"
											+ value + "</a></span>";
								}
							}
					...
					bbar : new Ext.PagingToolbar({
						xtype : 'paging',
						height : '30',
						store : 'OutStockPlanFormStore', // 主要是取store中从后台返回的totalCount，以及在store中设置的pagesize，就可以进行分页。具体的数据分页在后台的sql语句中进行。
...
Ext.onReady(function() {
	...
	Ext.getCmp('listwindow').setHeight(
			document.documentElement.clientHeight - 173);// 动态设置窗口的大小
	Ext.getCmp('listwindow1').setHeight(
			document.documentElement.clientHeight - 173);// 动态设置窗口的大小
});
...
Ext.EventManager.onWindowResize(function() {
	...
	fillBrowserWidth(gridwindow1);
	fillBrowserWidth(formwindow1);
	fillBrowser(outstockwd);
});
...
var add = function(btn) {
	var r = Ext.create('OutStockPlanFormModel', {});
...
var save = function(btn) {
	...
	if (records.length > 0) {
		...
		if (jsonArray.length > 0) {
			Ext.Ajax.request({
				url : '/WebWms/outstockplanform/save.action', // 保存
				method : 'POST',
				params : {
...
var del = function(btn) {
	...
	if (len > 0) {
		// 选中多行
		for (var i = 0; i < len; i++) {
			ids += rowsSelected[i].get("outstockplanform_pk") + ",";
		}
		Ext.Msg.show({
			title : '消息提示',
			message : '您确定删除数据?',
			...
			fn : function(btn) {// 用户点击交互后调用的函数，btn是指用户点了那个按钮
				if (btn === 'yes') {
					Ext.Ajax.request({
						url : "/WebWms/outstockplanform/delete.action", // 批量删除
						params : {
							'ids' : ids
						},
						success : function(response, options) {
...
var checktxt = function() {
	...
	var outstockplanform_pk2 = rowsSelected[0].get("outstockplanform_pk");
	OutStockFormContentStore.load({
		params : {
			start : 0,
			limit : 20,
			outstockplanform_pk2 : outstockplanform_pk2
		}
	});
	wd.show();
	wd.maximize();
