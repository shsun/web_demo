    	 ...
    	 [
	    	 {
                ...
                items: [
                {
                        ...
                        items: [
		                	{ xtype: 'label', width: 80,text: '入库计划单号',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请输入入库计划单号', name: 'instockplanform_number' },
				            { xtype: 'label', width: 80,text: '创单人员',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请输入创单人员',  name: 'user_name' },
				            { xtype: 'label', width: 80,text: '创单日期',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请输入创单日期', name: 'instockplanform_date' },
				            { xtype: 'label', width: 80,text: '状态',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请选择状态', name: 'instockplanform_state' }
		            	]
		          }
	            ]
	    	 },
	    	  {
                ...
                items: [
			            ...
			            { xtype: 'button', width: 100,height: 30, text: '查看订单内容',  style: 'margin-left:25px;',icon: "../../../../Images/btnimg/delete.png", handler: 'checktxt', scope: this },
...
Ext.onReady(function () {	
    ...
    //调整窗口的高度
     ...
     Ext.getCmp('listwindow').setWidth(document.documentElement.clientWidth);//动态设置窗口的大小
});
...
{	  
	...
	if(records.length>0){  
		...
		if(jsonArray.length>0)
	        {
	        	Ext.Ajax.request({
	        		url: '/WebWms/instockplanform/save.action', //保存
                    method:'POST',  
	    ...
	    if (len > 0) {
           ...
           Ext.Msg.show({
                ...
                fn: function (btn) {//用户点击交互后调用的函数，btn是指用户点了那个按钮
                    if (btn === 'yes') {
                        Ext.Ajax.request({
                        	url: '/WebWms/instockplanform/delete.action', //保存
                            params: { 'ids': ids },
                            success: function (response, options) {
                                var res = response.responseText;
                                if (res == null) {
...
var checktxt=function(){
   ...
   InStockPlanFormContentStore.load({ params: { start: 0, limit: 20, instockplanform_pk2: instockplanform_pk2} }); 
   wd.show();
   wd.maximize();
