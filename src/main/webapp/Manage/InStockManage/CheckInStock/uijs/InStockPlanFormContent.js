...
var win_save = function(btn)
{
	...
	if (records.length > 0)
	{
		...
		{
			Ext.Ajax.request({
						url : '/WebWms/inStockPlanFormContent/save.action', // 保存
						method : 'POST',
						params : {
	...
	if (len > 0)
	{
		...
		Ext.Msg.show({
			...
			{// 用户点击交互后调用的函数，btn是指用户点了那个按钮
				...
				{
					Ext.Ajax.request({
								url : '/WebWms/inStockPlanFormContent/delete.action', // 保存
								params : {
									'ids' : ids
								},
								success : function(response, options)
...
Ext.onReady(function()
		{
			...
			// 调整窗口的高度
			Ext.getCmp('listwindow')
					.setHeight(document.documentElement.clientHeight - 130);// 动态设置窗口的大小
			Ext.getCmp('listwindow')
					.setWidth(document.documentElement.clientWidth);// 动态设置窗口的大小
		});
