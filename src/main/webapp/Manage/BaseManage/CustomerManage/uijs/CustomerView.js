	...
	columns : 
		[
		 ...
		 bbar: new Ext.PagingToolbar({
		        xtype: 'paging',
		        height: '30',
		        store: 'CustomerStore', //主要是取store中从后台返回的totalCount，以及在store中设置的pagesize，就可以进行分页。具体的数据分页在后台的sql语句中进行。 store中的load函数中的pagesize设置数据显示的实际条数。
    	 ...
    	 [
	    	 {
                ...
                items: [
                {
                        ...
                        items: [
		                	{ xtype: 'label', width: 80,text: '客户编号',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请输入客户编号', name: 'customer_number' },
				            { xtype: 'label', width: 80,text: '客户姓名',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请输入客户姓名', name: 'customer_name' },
				            { xtype: 'label', width: 80,text: '客户邮箱',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请输入客户邮箱', name: 'customer_email' },
				            { xtype: 'label', width: 80,text: '客户电话',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请输入客户电话', name: 'customer_telephone' }
		            	]
		          },
		            {
                	    xtype: 'container',
                        ...
                        items: [
				            ...
				            { xtype: 'textfield', emptyText: '请输入客户手机', name: 'customer_mobilephone' },
				            { xtype: 'label', width: 80,text: '客户地址',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请输入客户地址', name: 'customer_address' },
				            { xtype: 'label', width: 80,text: '客户联系人',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请输入客户联系人', name: 'fk_linkman_pk' },
				            { xtype: 'label', width: 80,text: '客户备注',style: 'text-align:center;margin-left:10px;'},
				            { xtype: 'textfield', emptyText: '请输入客户备注', name: 'customer_remark' }
		            ]}
	            ]
	    	 },
	    	  {
...
var add=function (btn) {
    var r = Ext.create('CustomerModel', {
    	 customer_name:"",
    	 customer_email:"",
    	 customer_pk:"",
    	 customer_number:"",
...
var save=function(){
	...
	if(records.length>0){  
		...
		if(jsonArray.length>0)
	        {
	        	Ext.Ajax.request({
                    url: '/WebWms/customer/save.action', //保存
                    method:'POST',  
...
var del=function(btn){
	  var ids = "";
	    ...
	    if (len > 0) {
	        ...
	        for (var i = 0; i < len; i++) 
	        {
	        	ids += rowsSelected[i].get("customer_pk") + ",";
	        }
           Ext.Msg.show({
                title: '消息提示',
                message: '您确定删除数据?',
                ...
                fn: function (btn) {//用户点击交互后调用的函数，btn是指用户点了那个按钮
                    if (btn === 'yes') {
                        Ext.Ajax.request({
                            url: "/WebWms/customer/delete.action", //批量删除
                            params: { 'ids': ids },
                            success: function (response, options) {
                                var res = response.responseText;
                                if (res == null) {
